...
The debug writer receives its own copies after action resolution. `server/diagnostics/search.odin` publishes the latest two lifecycle-matched search records; it does not enrich beliefs from the host world. The arena search overlay reads that small file only when enabled. The AI Search tab uses the selected recorded trace and labels it as recorded. Neither UI can submit an AI decision. Senses inspectors read the separate observation-only feed for Vision and the existing search feed for their Exploration memory page.
...
`exploration_memory_layer.gd` draws that creature's remembered regions and self position on the shared `arena_overview.gd` frame, which fits the whole catalog arena north-up ([shared frames](dev-arena-overview.md)); the frame's outline is a developer reference, not remembered terrain. A square marks the region containing a visit; its dot marks the exact last occupied position. It does not imply full region coverage. Region numbers link the map to table rows and are not a chronological route. The `opponent_seen` marker belongs to the latest visit stored in that region, not to durable encounter learning. Empty space means unvisited or forgotten. There is no terrain/world query in this view.
