...
Implemented: one command starts a local Odin host, two Godot players, and optional audience windows directly in the selected arena. Game windows have the development FPS/ping overlay. Each creature runs its Observe AI (focused/peripheral vision, private memory, turning in place) on a dedicated native thread, with separate Godot decision and live-senses windows enabled by default (six windows total without an audience). Other senses, combat and learning follow later.
...
| `ARENA` | `meadow_crossing` | `meadow_crossing`, `sandbar`, `stone_garden`, `tiny_swords_village`; `make dev_vision` adds the staged `vision_range` QA arena and `make dev_scent` the staged `scent_trail` arena |
...
| `AI_DEBUG` | `1` | Opens AI inspectors; also sets the default for `SENSE_DEBUG`. Either diagnostic mode enables local telemetry, recording and arena cones |
| `SENSE_DEBUG` | `$(AI_DEBUG)` | Opens two live senses windows; set `0` to hide them, or `1` with `AI_DEBUG=0` for only senses companions |
...
make dev_vision P1=archer P2=orc   # close-quarters vision QA arena, same sensing/workers/actions
make dev_scent P1=archer P2=orc    # wide scent-trail QA arena; F8 shows the host scent heatmap
make dev_scent DEV_QA_SENSES=--qa-senses=client/dev/fixtures/content/blind_tracker.senses.json  # blind Orc: smell only
```

`--qa-senses` replaces `senses.json` in the private candidate with a QA catalog
(both the staged host and clients read it, so fingerprints agree). It never
touches the authored `client/content/data/senses.json`.

`make dev_vision` stages `client/dev/fixtures/content/vision_range.arenas.json` into the
private candidate's shared arena catalog (`--qa-arena`), so the staged host and clients
share one fingerprint and the recording stays replayable from the retained project. The
authored `client/content/data/arenas.json` is never modified. See the
[vision checkpoint](06g-focused-and-peripheral-vision.md) for the layout and manual steps.
...
Press **Ctrl+C** in the launcher terminal to close its host and windows. Closing a player or audience window also ends the run. Closing an AI or senses inspector leaves the arena and other inspectors running, removes it from reload waits, and keeps it closed across reloads until the next launcher run. Other running hosts/clients are left alone. To change the scenario arguments or replay after returning to Lobby, stop and rerun the command. The startup shortcut applies once per host lifetime.

The [AI debugger](06d-ai-debugger-harness.md) has its own timeline, branching trace,
event log, raw payload, Vision view and legacy spatial view. Pause and event/decision
...
[Checkpoint 6B.1.1](06l-live-senses-windows.md) provides two native senses windows,
one per creature, beside both decision debuggers. They show current vision data
...
works with F6 off. There is no stack trace or event-history browser here.
Pausing a decision debugger leaves its creature's senses window live.

[Arena cones and filters](06k-arena-sense-overlay.md) stay visible by default:
**F5** toggles cones, **F4** opens the separate **Senses · Perception** group, and
**F3** controls physical colliders. **F8** toggles the privileged host scent
heatmap in the **Senses · Olfaction** group; its class and smell-range filters
are saved per player window like the F6 setting ([olfaction](06n-olfactory-trails.md)). The senses window has independent focused and
peripheral drawing controls. A closed companion stays closed through the same
session's reloads. New generations bind fresh identities; lobby resets clear
current readings. Normal/release gameplay does not launch these windows.
...
| Shared `client/content/data/*.json` (including `senses.json` schema 2 and `terrains.json` schema 3) | Validate with both readers, then reopen host and clients against the same staged data; positions reset |
...
`ai_run` and `ai_trace_dir`. Senses companions have `senses_slots`, `senses_pids`
and `closed_senses`, with readiness, current readings and timing in
`senses1.json` / `senses2.json`. Each host start gets a fresh trace identity, including
...
128 MiB with visible capture status in each inspector. `AI_DEBUG=0` disables private diagnostics and QA recording by default; an explicit
`SENSE_DEBUG=1` keeps the live feed and recording enabled. [Daily cleanup](log-cleanup.md) removes logs
...
Normal `make dev_arena` and `make dev_vision` now run the private [opponent-search controller](06m-naturalistic-opponent-search.md). Press **F6** or use **F4 → Search / foraging**. This starts off, then persists independently for each player window across launcher restarts. The AI inspector has a Search tab with the same saved switch. Senses windows keep current observations and exploration memory on separate tabs; the memory view does not require F6. `client/presentation/target_acquired.tres` selects the target-found pixel asset.
...
walking or resting. After exhaustion, recover to 20% and release Space before
running again. Creatures give a small surprise hop when they find a target;
their shadows stay on the ground and the exclamation follows their heads.
See [running and the reaction](05c-trainer-running-and-target-alert.md).

For stationary receptor regression work, invoke `python3 tools/dev_session.py --p1 archer --p2 orc --arena vision_range --qa-arena client/dev/fixtures/content/vision_range.arenas.json --observe-only`. This QA option does not change the normal search default.
