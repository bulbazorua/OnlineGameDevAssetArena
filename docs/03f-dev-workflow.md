...
Implemented: one command starts a local Odin host, two Godot players, and optional audience windows directly in the selected arena. Every window has the development FPS/ping overlay. The arena currently supports movement; combat and AI remain later phases.
...
| --- | --- | --- |
| `P1`, `P2` | `circle`, `square` | `circle`, `square`, `triangle`, `diamond`; mirror matches allowed |
...
Press **Ctrl+C** in the launcher terminal to close its host and windows. Closing any owned window also ends the run. Other running hosts/clients are left alone. To change the scenario arguments or replay after returning to Lobby, stop and rerun the command. The startup shortcut applies once per host lifetime.
