# Arena protocol, version 5

Checkpoint **3D** implements Lobby → Selecting → Countdown → InArena, including player movement and audience updates. Restart clients and the host together after updating from version 4.
...
| 2 Welcome | Host → joining client | Assigned role `u8` | 7 | 0 |
...
| 8 CommandRejected | Host → sender | Current round `u32`, rejected kind `u8`, reason `u8` | 12 | 0 |
...
Authority comes from the welcomed connection. Commands never contain a claimed owner or position. Duplicate Hello returns the same Welcome and current SessionState without changing membership. Audience members must reconnect to claim a vacant player slot; there is no automatic promotion.
...
Welcome precedes the first full state on channel 0. Every joined player and viewer receives a full state when membership, phase, a pick, readiness, or the displayed countdown number changes. Late viewers receive the current countdown or current live positions directly.
...
- The second Ready enters Countdown for **300 simulation ticks**. The host publishes 5 immediately, then 4, 3, 2, 1 at one-second intervals. At zero it creates two characters at the map's spawn centers and enters InArena.
...
- A fighter departure resets everyone to Lobby. Audience departure only changes the count.
...
Movement uses the selected character's circular footprint against blocked terrain and boundaries. X then Y collision resolution permits wall sliding. Grass, ground, and sand are walkable; water and stone block movement. Terrain bonuses and character-to-character collision are deferred.
