...
func export_character(context: Dictionary) -> CharacterExports:
	...
	if not authored is Dictionary or authored.get("key") != "archer" or not authored.get("display_name") is String:
		context.sources.fail("archer_definition", path, "Invalid Archer identity")
		return null
	var result := CharacterExports.new()
	result.module_key = "archer"
	result.contract_id = "character.basic_combat"
	result.contract_version = "1.0.0-draft.1"
	result.export_api_version = "0.1.0"
	result.gameplay_definition = {"identity": {"key": "archer", "display_name": authored.display_name},
	...
	# Arrow.png is reserved for the projectile extension.
	var roles := {"idle": "bow_rest", "walk": "bow_run", "attack": "bow_shoot", "hurt": "bow_hurt", "death": "bow_fall"}
