...
content_validates_and_matches_digest_fixture :: proc(t: ^testing.T) {
    ...
    testing.expect(t, catalog.fingerprint == CONTENT_FIXTURE_DIGEST)
    testing.expect(t, len(catalog.characters) == 1 && catalog.characters[0].key == "circle" && catalog.characters[0].footprint_radius == 12)
    destroy(&catalog)
    numeric := `{"schema_version":1.0,"characters":[{"id":1e0,"key":"circle","display_name":"Circle","footprint_radius":12.5}]}`
    testing.expect(t, parse_characters(&catalog, transmute([]u8)numeric))
    testing.expect(t, catalog.characters[0].id == 1 && catalog.characters[0].footprint_radius == 12.5)
    destroy(&catalog)
}
...
content_rejects_invalid_definitions_and_releases_partial_loads :: proc(t: ^testing.T) {
    for invalid in ([]string{
        ...
        catalog: Game_Content
        testing.expect(t, !parse_characters(&catalog, transmute([]u8)invalid), invalid)
        testing.expect(t, len(catalog.characters) == 0 && catalog.fingerprint == [32]u8{})
        destroy(&catalog)
    }
}
...
content_test_senses :: proc(senses: string, characters := CONTENT_FIXTURE) -> (ok: bool) {
    ...
    if !parse_characters(&catalog, transmute([]u8)characters) { return false }
    return parse_extra(&catalog, transmute([]u8)senses, .Senses)
}
...
sense_catalog_validates_profiles_bindings_and_bounds :: proc(t: ^testing.T) {
    catalog: Game_Content
    testing.expect(t, parse_characters(&catalog, transmute([]u8)string(CONTENT_FIXTURE)))
    testing.expect(t, parse_extra(&catalog, transmute([]u8)string(SENSES_FIXTURE), .Senses))
    testing.expect(t, len(catalog.senses) == 1 && catalog.senses[0].key == "starter_senses" && catalog.senses[0].vision == {true, 256, 60, 160, 6})
    testing.expect(t, catalog.senses[0].olfaction == {true, 160, 12, true} && catalog.senses[0].emitter == {true, .Human, 1})
    testing.expect(t, catalog.characters[0].sense_profile == "starter_senses" && catalog.characters[0].vision.range == 256)
    testing.expect(t, catalog.characters[0].olfaction.range == 160 && catalog.characters[0].emitter.class == .Human && catalog.trainer_emitter == {true, .Human, 1})
    destroy(&catalog)
    valid_edges := []string{
...
terrain_catalog_requires_schema_three_with_explicit_sight_and_scent_rules :: proc(t: ^testing.T) {
    for terrains in ([]string{
        ...
        catalog: Game_Content
        testing.expect(t, !parse_extra(&catalog, transmute([]u8)terrains, .Terrains), terrains)
        destroy(&catalog)
    }
    catalog: Game_Content
    defer destroy(&catalog)
    testing.expect(t, parse_extra(&catalog, transmute([]u8)string(`{"schema_version":3,"terrains":[{"id":4,"key":"water","display_name":"Water","symbol":"w","walkable":false,"blocks_vision":false,"scent":"water"},{"id":5,"key":"stone","display_name":"Stone","symbol":"#","walkable":false,"blocks_vision":true,"scent":"solid"},{"id":9,"key":"tall_grass","display_name":"Tall grass","symbol":"t","walkable":true,"blocks_vision":true,"scent":"open"}]}`), .Terrains))
    testing.expect(t, !catalog.terrains[0].walkable && !catalog.terrains[0].blocks_vision && catalog.terrains[0].scent == .Water)
    testing.expect(t, catalog.terrains[1].blocks_vision && catalog.terrains[1].scent == .Solid)
    // Scent is authored on its own: sight-blocking tall grass can still carry scent.
    testing.expect(t, catalog.terrains[2].blocks_vision && catalog.terrains[2].scent == .Open)
}
