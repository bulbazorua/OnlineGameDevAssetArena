...
search_reset_places_both_creatures_out_of_sight_on_all_shipped_and_qa_maps :: proc(t: ^testing.T) {
    ...
    battle_test_content_with_qa(t, &content)
    ...
    for &arena in content.arenas {
        ...
        placements, valid := dev_search_reset_placements(&sim.session, &content)
        testing.expectf(t, valid, "No separated search placement in %s", arena.key)
        if !valid { continue }
        for placement, index in placements {
            definition := content_character(&content, sim.session.players[index].character_id)
            other := content_character(&content, sim.session.players[1 - index].character_id)
            testing.expect(t, arena_position_is_clear(&arena, &content, placement.creature, definition.footprint_radius))
            testing.expect(t, arena_position_is_clear(&arena, &content, placement.trainer, TRAINER_RADIUS))
...
search_reset_is_authorized_atomic_and_clears_both_private_lifecycles :: proc(t: ^testing.T) {
    ...
    for tick in 0..<1500 { simulation_tick(&sim, &content) }
    ...
    changed, rejected := session_apply(&sim.session, &content, 0, command, true)
    testing.expect(t, !changed && rejected == .Audience_Read_Only && sim.session == before)
    changed, rejected = session_apply(&sim.session, &content, 1, command)
    testing.expect(t, !changed && rejected == .Dev_Only && sim.session == before)
    changed, rejected = session_apply(&sim.session, &content, 1, command, true)
    when !ODIN_DEBUG {
        testing.expect(t, !changed && rejected == .Dev_Only && sim.session == before)
        return
    }
    ...
    battle_sync(&sim.battle, &sim.session, &content, sim.seed, .Search)
    ...
    fresh := sim.session
    changed, rejected = session_apply(&sim.session, &content, 2, command, true)
    ...
    for tick in 0..<96 { simulation_tick(&sim, &content) }
...
search_reset_rejects_small_or_disconnected_ground_without_changing_the_match :: proc(t: ^testing.T) {
    ...
    before := sim.session
    definition := content_character(&content, 5)
    definition.vision.range = 2048
    _, valid := dev_search_reset_placements(&sim.session, &content)
    testing.expect(t, !valid && sim.session == before)
    when ODIN_DEBUG {
        changed, rejection := session_apply(&sim.session, &content, 1, {kind = .Dev_Reset_Search, round_id = before.round_id}, true)
