...
qa_arena_geometry_matches_the_documented_setup :: proc(t: ^testing.T) {
    ...
    testing.expect(t, p1.position == arena_cell_center(arena, {7, 5}) && p2.position == arena_cell_center(arena, {12, 10}))
    testing.expect(t, p1.facing == .East && p2.facing == .West)
    testing.expect(t, sim.session.trainers[0].position == arena_cell_center(arena, {5, 5}) && sim.session.trainers[1].position == arena_cell_center(arena, {14, 10}))
    ...
    testing.expect(t, !perception.line_of_sight(grid, p1_eye, arena_cell_center(arena, {14, 8})), "wall should hide (14,8) from P1")
    testing.expect(t, perception.line_of_sight(grid, p1_eye, arena_cell_center(arena, {14, 6})), "(14,6) is visible past the wall")
    testing.expect(t, perception.line_of_sight(grid, p1_eye, arena_cell_center(arena, {11, 1})), "water pool does not block sight")
    testing.expect(t, arena_cell_is_blocked(arena, &content, {10, 2}) && !arena_cell_blocks_sight(arena, &content, {10, 2}))
}
...
sampling_uses_frozen_poses_with_independent_deadlines_and_slot_order_independence :: proc(t: ^testing.T) {
    ...
    map_id := battle_test_content_with_qa(t, &content)
    defer content_destroy(&content)
    sim := battle_test_scenario(&content, map_id, 6)
    ...
    straight := senses_prepare(&original.battle, &original.session, &content, true)
    swapped := senses_prepare(&copied.battle, &copied.session, &content, true)
    ...
    locked := battle_test_scenario(&content, map_id, 5)
    simulation_tick(&locked, &content)
    ...
    for _ in 0..<95 { simulation_tick(&locked, &content) }
