...
receptor_bind :: proc(receptor: ^Receptor, definition: ^Character_Definition) {
    receptor^ = {}
...
// scent updates and before either creature acts. A non-due receptor hands over its
// last sample unchanged, so one sense never refreshes another sense's clock.
senses_prepare :: proc(battle: ^Battle_Runtime, session: ^Session, content: ^Game_Content, can_act: bool) -> (inputs: [MAX_PLAYERS]obs.Sense_Input) {
    world := Vision_World{characters = session.characters, trainers = session.trainers}
    if arena := content_arena(content, session.map_id); arena != nil { world.grid = arena_opacity_grid(arena) }
    for index in 0..<MAX_PLAYERS {
        receptor := &battle.receptors[index]
        input := &inputs[index]
        ...
        input.olfaction, input.olfaction_is_new = olfaction_receptor_sample(&receptor.olfaction, &battle.scent.field, world.characters[index], session.round_id, session.server_tick, can_act)
    }
    return
}
