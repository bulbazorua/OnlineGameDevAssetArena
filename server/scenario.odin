...
dev_scenario_load :: proc(options: Options, catalog: ^content.Game_Content) -> (scenario: Dev_Scenario, valid: bool) {
    ...
    for key, index in ([2]string{options.dev_p1, options.dev_p2}) {
        ...
        if scenario.character_ids[index] == 0 {
            ...
            for definition in catalog.characters { fmt.eprintfln("  %s", definition.key) }
            return {}, false
        }
    }
    for arena in catalog.arenas { if arena.key == options.dev_arena { scenario.map_id = arena.id; break } }
    if scenario.map_id == 0 {
        fmt.eprintfln("[dev] Unknown arena '%s'. Available:", options.dev_arena)
        for arena in catalog.arenas { fmt.eprintfln("  %s", arena.key) }
        return {}, false
    }
...
dev_scenario_start :: proc(scenario: ^Dev_Scenario, session: ^simulation.Session, catalog: ^content.Game_Content) -> bool {
    if !scenario.enabled || scenario.applied || simulation.session_player_mask(session) != 3 || session.phase != .Lobby { return false }
    scenario.applied = true
    simulation.session_start_match(session, catalog, scenario.character_ids, scenario.map_id, scenario.countdown_seconds)
