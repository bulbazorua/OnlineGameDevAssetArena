...
trainer_run_test_ground :: proc(t: ^testing.T, content: ^Game_Content) -> ^Arena_Definition {
...
trainer_running_lasts_five_seconds_and_exhaustion_requires_recovery_and_release :: proc(t: ^testing.T) {
    ...
    arena := trainer_run_test_ground(t, &content)
    ...
    for step in u32(0)..<308 {
        ...
        trainer_tick_motion(&trainer, 100 + step, arena, &content)
...
trainer_run_preserves_movement_clock_collision_and_stationary_energy :: proc(t: ^testing.T) {
    ...
    for tick in u32(100)..=108 { trainer_tick_motion(&trainer, tick, arena, &content) }
    testing.expect(t, trainer.position == [2]f32{258, 256} && trainer.energy == 600)
    trainer.input_mask = 2 | TRAINER_RUN_INPUT
    trainer_tick_motion(&trainer, 109, arena, &content)
    ...
    trainer.input_mask = 2
    trainer_tick_motion(&trainer, 110, arena, &content)
    ...
    trainer_tick_motion(&trainer, 10, arena, &content)
    ...
    for mask in ([]u8{16, 19, 28}) {
        ...
        for tick in u32(0)..<30 { trainer_tick_motion(&trainer, tick, arena, &content) }
    ...
    for age in u32(0)..=8 { trainer_tick_motion(&trainer, u32(0xfffffffc) + age, arena, &content) }
...
trainer_energy_is_host_timed_private_to_each_trainer_and_reset_with_the_round :: proc(t: ^testing.T) {
    content: Game_Content
    _ = trainer_run_test_ground(t, &content)
    defer content_destroy(&content)
    session: Session
    movement_test_ready(&session, &content)
    session_enter_arena(&session, &content)
    for sequence in u32(1)..=1000 {
        session_apply(&session, &content, 1, {kind = .Input, round_id = session.round_id, input_sequence = sequence, input_mask = 18})
    }
    testing.expect(t, session.trainers[0].energy == 600 && session.trainers[0].input_mask == 0)
    session.summon_elapsed_ticks = 90
    session_apply(&session, &content, 1, {kind = .Input, round_id = session.round_id, input_sequence = 1001, input_mask = 18})
    for _ in 0..<20 { session_tick(&session, &content) }
