...
Trainer :: Character // Shared spatial/input record; separate collection and control path.

trainer_tick_motion :: proc(trainer: ^Trainer, tick: u32, arena: ^Arena_Definition, content: ^Game_Content) {
    next := character_move(trainer.position, trainer.input_mask, TRAINER_RADIUS, arena, content)
    ...
    if tick - trainer.state_start_tick >= TRAINER_WALK_START_TICKS { trainer.position = next }
}
