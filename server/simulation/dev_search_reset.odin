...
// A fresh round on validated, separated ground. The host logs the result; nothing here prints.
@(private)
dev_search_reset :: proc(session: ^Session, catalog: ^content.Game_Content, enabled: bool) -> (bool, Command_Reject_Reason) {
    ...
    placements, valid := dev_search_reset_placements(session, catalog)
...
@(private)
dev_search_reset_placements :: proc(session: ^Session, catalog: ^content.Game_Content) -> (result: [2]Dev_Search_Spawn, valid: bool) {
    arena := content.find_arena(catalog, session.map_id)
    if arena == nil { return }
    radius, vision_range := TRAINER_RADIUS, f32(0)
    for player in session.players {
        definition := content.find_character(catalog, player.character_id)
    ...
    reachable := dev_search_reset_reachable(arena, catalog, radius)
    ...
    for reached, index in reachable {
        ...
        if candidate, fits := dev_search_reset_spawn(arena, catalog, reachable, cell, radius); fits { append(&candidates, candidate) }
    }
    if len(candidates) < 2 { return }
    origin := content.arena_cell_center(arena, arena.spawns[0])
...
@(private)
dev_search_reset_spawn :: proc(arena: ^content.Arena_Definition, catalog: ^content.Game_Content, reachable: []bool, cell: [2]int, radius: f32) -> (Dev_Search_Spawn, bool) {
    ...
    for offset in ([4][2]int{{0, 1}, {0, -1}, {-1, 0}, {1, 0}}) {
        ...
        if !reachable[neighbor.y * arena.width + neighbor.x] || !content.arena_spawn_is_clear(arena, catalog, neighbor, TRAINER_RADIUS) { continue }
        trainer := content.arena_cell_center(arena, neighbor)
        if content.arena_elevation_at(arena, cell) != content.arena_elevation_at(arena, neighbor) { continue }
...
@(private)
dev_search_reset_reachable :: proc(arena: ^content.Arena_Definition, catalog: ^content.Game_Content, radius: f32) -> []bool {
    reached := make([]bool, arena.width * arena.height)
    queue := make([dynamic][2]int)
    defer delete(queue)
    first := arena.spawns[0]
    if !content.arena_spawn_is_clear(arena, catalog, first, radius) { return reached }
    reached[first.y * arena.width + first.x] = true
    append(&queue, first)
    for index := 0; index < len(queue); index += 1 {
        cell := queue[index]
        from := content.arena_cell_center(arena, cell)
        for offset in DEV_SEARCH_NEIGHBORS {
            neighbor := cell + offset
            if neighbor.x < 0 || neighbor.y < 0 || neighbor.x >= arena.width || neighbor.y >= arena.height { continue }
            next := neighbor.y * arena.width + neighbor.x
            if reached[next] || !content.arena_spawn_is_clear(arena, catalog, neighbor, radius) { continue }
            to := content.arena_cell_center(arena, neighbor)
            if !content.arena_position_is_clear(arena, catalog, (from + to) * 0.5, radius) || !content.arena_step_is_allowed(arena, catalog, from, to) { continue }
            reached[next] = true
