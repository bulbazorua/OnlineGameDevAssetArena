...
@(private)
scent_environment_bind :: proc(environment: ^Scent_Environment, session: ^Session, catalog: ^content.Game_Content) {
    environment^ = {}
    ...
    for index in 0..<MAX_PLAYERS { scent_environment_bind_creature(environment, session, catalog, index) }
    for trainer, index in session.trainers { scent_emitter_arm(&environment.emitters[MAX_PLAYERS + index], trainer.entity_id, trainer.position, catalog.trainer_emitter) }
}

// Only this creature's emitter starts over; the ground keeps every old deposit.
@(private)
scent_environment_bind_creature :: proc(environment: ^Scent_Environment, session: ^Session, catalog: ^content.Game_Content, index: int) {
    character := session.characters[index]
    emitter: obs.Scent_Emitter
    if definition := content.find_character(catalog, character.definition_id); definition != nil { emitter = definition.emitter }
...
@(private)
scent_environment_tick :: proc(environment: ^Scent_Environment, session: ^Session, catalog: ^content.Game_Content, emitting: bool) {
    ...
    for trainer, index in session.trainers {
        scent_emitter_follow(field, &environment.emitters[MAX_PLAYERS + index], trainer.entity_id, trainer.position, catalog.trainer_emitter, emitting)
    }
