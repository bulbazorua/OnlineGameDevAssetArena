...
live_senses_drop_old_lifecycles_without_waiting_for_new_decisions :: proc(t: ^testing.T) {
    ...
    testing.expect(t, sense_collect(debug, &records) == 2)
    testing.expect(t, records[0].delivered_us == 123 && records[0].scent_delivered_us == 456)
    debug.sense_world.entities[0] = 99
    testing.expect(t, sense_collect(debug, &records) == 1 && records[0].owner_id == 2)
    debug.sense_world.round_id += 1
    testing.expect(t, sense_collect(debug, &records) == 0)
    debug.sense_world.round_id -= 1
...
live_senses_envelope_fits_bounded_reader_at_maximum_evidence :: proc(t: ^testing.T) {
    ...
    records := [simulation.MAX_PLAYERS]Sense_Record{sense_record(&widest, 1000, 1000), sense_record(&widest, 1000, 1000)}
    records[0].owner_id = 1
    snapshot := Sense_Snapshot{schema_version = SENSE_SCHEMA, run_id = "run/generation-100/attempt-100",
    ...
    defer delete(data)
    testing.expect(t, error == nil && len(data) <= SENSE_BYTES)
}
