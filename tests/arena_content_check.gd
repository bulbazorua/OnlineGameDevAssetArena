...
func _run() -> void:
	...
	_expect(content.arena_catalog.arenas.size() == 3 and content.arena_catalog.terrains.size() == 5, "Expected three arenas and five terrain types.")
	...
	_expect(seen.size() == 5, "Shipped maps do not demonstrate every terrain type.")
	world.queue_free()
	await process_frame
	_expect(GameProtocol.encode_command(GameProtocol.MessageKind.SELECT_ARENA, 0x04030201, 0, false, 2) == PackedByteArray([79, 71, 65, 65, 5, 9, 1, 2, 3, 4, 2, 0]), "SelectArena bytes differ from Odin.")
	_expect(GameProtocol.encode_command(GameProtocol.MessageKind.SET_READY, 0x04030201, 4, true, 2) == PackedByteArray([79, 71, 65, 65, 5, 6, 1, 2, 3, 4, 4, 0, 2, 0, 1]), "SetReady bytes differ from Odin.")
	if failures.is_empty():
		print("PASS: full content digest, terrain/map validation, all 840 rendered cells, terrain metadata, coordinates, spawn clearance, and arena command fixtures.")
		quit(0)
	else:
